...
describe("format option", () => {
    it("with default option", () => {
        const config = format()
        assert.equal(config.src, "./package.json")
        assert.equal(config.dest, "./dist")
    ...
    it("with filename", () => {
        const config = format({ dest: "./dist/client", filename: "some.json" })
        assert.notEqual(config.srcSet, undefined)
        assert.notEqual(config.destSet, undefined)
        assert.equal(config.srcSet.filename, "package.json")
